...
# 예측 R,T,C / 타겟 객체의 포즈 / 객체 몇몇 포인트 / 객체 번호 / masked 포인트 클라우드 / lr / refine mode
def loss_calculation(pred_r, pred_t, pred_c, target, model_points, idx, points, w, refine, num_point_mesh, sym_list):
    ...
    pred = torch.add(torch.bmm(model_points, base), points + pred_t) # 예측 포즈

    if not refine:
        if idx[0].item() in sym_list: # 대칭 객체일 경우
            target = target[0].transpose(1, 0).contiguous().view(3, -1)
            pred = pred.permute(2, 0, 1).contiguous().view(3, -1)
            inds = knn.forward(target.unsqueeze(0), pred.unsqueeze(0)) # knn으로 가까운 포인트 찾음
            target = torch.index_select(target, 1, inds.view(-1).detach() - 1)
            target = target.view(3, bs * num_p, num_point_mesh).permute(1, 2, 0).contiguous()
            pred = pred.view(3, bs * num_p, num_point_mesh).permute(1, 2, 0).contiguous()
    
    dis = torch.mean(torch.norm((pred - target), dim=2), dim=1) # 논문 equ2
    loss = torch.mean((dis * pred_c - w * torch.log(pred_c)), dim=0) # 논문 equ3
    ...
    new_points = torch.bmm((points - ori_t), ori_base).contiguous() # masked 포인트 클라우드 돌려주는 부분
