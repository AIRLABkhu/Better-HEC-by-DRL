...
class DenseFusionInferenceRos:
    def __init__(self, args, single_frame_mode=True):
        ...
        root_dir = '/home/airlab/test_ws/src/Better-HEC-by-DRL/DenseFusion_Pytorch_1_0/weights/dis'
        
        self.dataset_config_dir = '/home/airlab/test_ws/src/Better-HEC-by-DRL/DenseFusion_Pytorch_1_0/datasets/linemod/dataset_config'
    ...
    def adaptive_reward(self, loss, raw_action_pose):  

        if raw_action_pose == 12:
            if loss > 0.004:
                reward = 1
            elif 0.004>= loss and loss > 0.002:
                reward = 2
            elif 0.002>= loss and loss > 0.001:
                reward = 4
            else:
                reward = 8
        else:
    ...
    def draw_bounding_box(self, model_ply):
        ...
        self.next_state = [pred_coord[0],pred_coord[1],pred_coord[2],pred_coord[3],pred_coord[4],pred_coord[5], offset-1]

        self.c_loss = loss

        self.reward = self.adaptive_reward(loss, raw_action_pose)
...
if __name__ == '__main__':
    ...
    pt = ply_vtx('/home/airlab/test_ws/src/Better-HEC-by-DRL/DenseFusion_Pytorch_1_0/datasets/linemod/Linemod_preprocessed/models/obj_16.ply')
    ...
    while not rospy.is_shutdown():
        ...
        if offset == 0:
            ...
            previous_r = RR.from_euler('xyz', [next_state[3], next_state[4], next_state[5]], degrees=True)
            previous_r = previous_r.as_matrix()
            previous_t = np.array([next_state[0], next_state[1], next_state[2]])
            previous_rt= np.column_stack((previous_r,previous_t))
            previous_rt= np.row_stack((previous_rt, np.array([0.0, 0.0, 0.0, 1.0])))

            action = np.add(action, next_state[:6])
            current_r = RR.from_euler('xyz', [action[3], action[4], action[5]], degrees=True)
            current_r = current_r.as_matrix()
            current_t = np.array([action[0], action[1], action[2]])
            current_rt= np.column_stack((current_r,current_t))
            current_rt= np.row_stack((current_rt, np.array([0.0, 0.0, 0.0, 1.0])))


            cam_to_ee_rt = np.linalg.inv(previous_rt) @ current_rt


            listener.waitForTransform("/base_link", "/end_effector_link", rospy.Time(0), rospy.Duration(3.0))
            (trans,rot) = listener.lookupTransform('/base_link', '/end_effector_link', rospy.Time(0))
            r = np.array([rot[0],rot[1],rot[2],rot[3]])
            base_to_ee_r = get_quaternion_rotation_matrix(r, switch_w=True)
            base_to_ee_t = np.array([trans[0], trans[1], trans[2]])
            base_to_ee_rt = np.column_stack((base_to_ee_r,base_to_ee_t))
            base_to_ee_rt= np.row_stack((base_to_ee_rt, np.array([0.0, 0.0, 0.0, 1.0])))

            next_base_to_ee_rt = base_to_ee_rt @ cam_to_ee_rt

            listener.waitForTransform("/end_effector_link", "/tool_frame", rospy.Time(0), rospy.Duration(3.0))
            (trans,rot) = listener.lookupTransform('/end_effector_link', '/tool_frame', rospy.Time(0))
            r = np.array([rot[0],rot[1],rot[2],rot[3]])
            ee_to_tool_r = get_quaternion_rotation_matrix(r, switch_w=True)
            ee_to_tool_t = np.array([trans[0], trans[1], trans[2]])
            ee_to_tool_rt = np.column_stack((ee_to_tool_r,ee_to_tool_t))
            ee_to_tool_rt= np.row_stack((ee_to_tool_rt, np.array([0.0, 0.0, 0.0, 1.0])))

            next_base_to_tool_rt = next_base_to_ee_rt@ee_to_tool_rt

            next_base_to_tool_r = RR.from_matrix(next_base_to_tool_rt[:3,:3])
            next_base_to_tool_r = next_base_to_tool_r.as_euler('xyz', degrees=True)
            next_base_to_tool_t = np.array([next_base_to_tool_rt[0,3], next_base_to_tool_rt[1,3], next_base_to_tool_rt[2,3]])
            next_base_to_tool_rt = np.array([next_base_to_tool_t[0],next_base_to_tool_t[1],next_base_to_tool_t[2],next_base_to_tool_r[0],next_base_to_tool_r[1],next_base_to_tool_r[2]])
            
            # print("next_pose", next_base_to_tool_rt)
