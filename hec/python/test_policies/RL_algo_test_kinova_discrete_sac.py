...
def act(cur_state):
    ...
    # raw_action =  random.randrange(0,13)
    #scale of actions
    if raw_action == 0:
...
def step(req):
    ...
    response.raw_action = np.array(raw_action)
    return response
...
def step2():
    ...
    try:
        ...
        return np.zeros((7,)), 0, 0, np.zeros((6,)), 100, 0 , 0, 0, 0, 0
    return resp1.next_state, resp1.reward, resp1.done, resp1.action_srv, resp1.raw_action_srv, resp1.loss , resp1.ee_rot_loss, resp1.ee_trans_loss, resp1.hand_rot_loss, resp1.hand_trans_loss
...
if __name__ == "__main__":
    ...
    while not rospy.is_shutdown():
        if episode >= max_frames:
            data_array = np.asarray(data_list)
            print(data_array[-1])
            np.save("experiment_data_kinova_loss.npy",data_loss)
            np.save("experiment_data_kinova_raw_action.npy",data_raw_action)
            np.save("experiment_data_kinova_ee_rot_loss.npy",data_ee_rot_loss)
            np.save("experiment_data_kinova_ee_trans_loss.npy",data_ee_trans_loss)
            np.save("experiment_data_kinova_hand_rot_loss.npy",data_hand_rot_loss)
            np.save("experiment_data_kinova_hand_trans_loss.npy",data_hand_trans_loss)
            np.save("experiment_data_kinova.npy",data_array[-1])
            rospy.signal_shutdown('Shutting Down')
        
        if i==0 and j==0:
            state, org_reward, done, action_srv1, raw_action_srv, loss , ee_rot_loss, ee_trans_loss, hand_rot_loss, hand_trans_loss = step2()
        ...
        step(None)
        next_state, reward, done, action_srv, raw_action_srv, loss, ee_rot_loss, ee_trans_loss, hand_rot_loss, hand_trans_loss  = step2()
        ...
        if raw_action_srv != 12:
            if state[0] == next_state[0] and state[1] == next_state[1] and state[2] == next_state[2] and state[3] == next_state[3] and state[4] == next_state[4] and state[5] == next_state[5] and state[6] == next_state[6]:
                continue
        ...
        actions_lst.append(action_srv)
        raw_actions_lst.append(raw_action_srv)
        next_state_lst.append(next_state)
        reward_lst.append(reward)
        loss_lst.append(loss)
        ee_rot_loss_lst.append(ee_rot_loss)
        ee_trans_loss_lst.append(ee_trans_loss)
        hand_rot_loss_lst.append(hand_rot_loss)
        hand_trans_loss_lst.append(hand_trans_loss)
        ...
        rospy.loginfo(rospy.get_caller_id() + 'got next_state %s',next_state)
        ...
        if done or i==max_steps:
            ...
            episode_len = i
            data.append(episode)
            data.append(state_lst)
            data.append(actions_lst)
            data.append(raw_actions_lst)
            data.append(reward_lst)
            data.append(next_state_lst)
            data.append(loss_lst)
            data.append(ee_rot_loss_lst)
            data.append(ee_trans_loss_lst)
            data.append(hand_rot_loss_lst)
            data.append(hand_trans_loss_lst)
        ...
        if done or i>=max_steps:
            ...
            data_loss.append(loss_lst)
            data_raw_action.append(raw_actions_lst)
            data_ee_rot_loss.append(ee_rot_loss_lst)
            data_ee_trans_loss.append(ee_trans_loss_lst)
            data_hand_rot_loss.append(hand_rot_loss_lst)
            data_hand_trans_loss.append(hand_trans_loss_lst)
            episode_reward = 0
            last = 0
            done = False
            episode += 1
